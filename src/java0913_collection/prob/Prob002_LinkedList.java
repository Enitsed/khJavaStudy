...
public class Prob002_LinkedList {
	...
	public static void main(String[] args) {
		...
		Scanner sc = null;
		try {
			sc = new Scanner(new File("src/java0913_collection/prob/hospital.txt"));
			while (sc.hasNextLine()) {
				data = sc.nextLine().split("/");
				Doctor doc = new Doctor(data[0], data[1], Integer.parseInt(data[2]));
				queue.offer(doc);
			}
	...
	public static void prnDisplay(LinkedList<Doctor> queue) {
		...
		/*
		 * 출력결과 [김병조 의사] 진료과목 : 외과 환자수 : 39 [이상만 의사] 진료과목 : 내과 환자수 : 50 [박상기 의사] 진료과목 :
		 * 피부과 환자수 : 20
		 */
